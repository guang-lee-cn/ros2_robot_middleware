...
|:---:|------|:---:|:---:|
| 一 | 基础元信息 | ❌ 缺 | P1 |
| 二 | 业务与需求背景 | ⚠️ 散落 PRD/doc | P2 |
| 三 | 系统边界与上下文 | ⚠️ 部分在 ARCHITECTURE | P1 |
| 四 | 总体架构设计 | ✅ ARCHITECTURE.md + ADR | — |
| 五 | 分模块详细设计 | ✅ subsystems/ 6 文档 | — |
| 六 | 跨模块核心机制 | ⚠️ 观测/HAL 在 guides/，缺通信/并发/配置 | P2 |
| 七 | 数据架构设计 | ❌ 缺 | P3 |
| 八 | 接口设计规范 | ⚠️ 只有 DDS Topic 表 | P2 |
| 九 | 非功能专项设计 | ⚠️ 散落各文档 | P1 |
| 十 | 部署与运维架构 | ❌ 缺 | P3 |
| 十一 | 风险与应对方案 | ❌ 缺 | P2 |
| 十二 | 测试架构设计 | ✅ quality/README | — |
| 十三 | 落地实施规划 | ⚠️ 迭代计划在此文档 | — |
| 十四 | 附录 | ⚠️ 未集中 | P3 |
...
- [ ] 项目定位：AMR 中间件展示项目，面向 ROS2/DDS 中间件工程师面试
- [ ] 核心功能清单（Must/Should/Could）
- [ ] NFR 约束表（延迟、吞吐、可靠性、安全、资源）

### 六、跨模块核心机制（补充缺失模块文档）

- [ ] `subsystems/communication.md` — DDS 域划分、QoS 分级、服务发现、跨设备通信（当前仅在 ADR-8 中提及）
- [ ] 并发与同步机制 → 并入 `九、非功能` 的实时性保障
- [ ] 配置管理机制 → `subsystems/configuration.md` — sensors.yaml、Fast-DDS XML profile、ROS2 params

### 八、接口设计规范（新增文档）

- [ ] `doc/interfaces.md` — 完整 DDS Topic 表（类型/QoS/周期/方向）
- [ ] 函数 API 约束（智能指针所有权规范、参数传递规则）
- [ ] Action 接口规范（Goal→Feedback→Result 生命周期）
- [ ] 接口版本兼容策略

### 十一、风险与应对方案（新增文档）

- [ ] 架构风险矩阵：通信延迟超标、内存泄漏、DDS 消息丢失、日志阻塞
- [ ] 技术风险矩阵：第三方库兼容性、编译器链接问题
- [ ] 每条含：描述、影响等级、规避方案、应急预案
